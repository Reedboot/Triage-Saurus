...
`enrich_findings.py --experiment <id>` queries `findings WHERE llm_enriched_at IS NULL` and calls you once per finding. You return JSON (authoritative schema lives in `Scripts/Enrich/enrich_findings.py`):
```json
{
