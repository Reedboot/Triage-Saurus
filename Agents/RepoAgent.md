...
**Phase 1 rule integration:** Phase 1 should use the per-service context rules under `Rules/Detection/` (e.g., storage, SQL, Key Vault detectors) so downstream scans know exactly which security rules to prioritize. If opengrep is unavailable, fall back to the manual patterns in `Rules/Summary.md`, but always aim for deterministic per-service detection.
