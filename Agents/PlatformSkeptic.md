...
3. **Access the IaC/config files** referenced in `source_file:source_line_start` to verify or challenge
...
**Idempotency:** If `skeptic_reviews` already has a row for `(finding_id, 'platform')`, skip — do not re-review.

**Also update the finding MD file** under `Output/Learning/experiments/<id>/Findings/` (legacy format still used for reports), adding feedback under `## 🤔 Skeptic` → `### 🏗️ Platform`.
