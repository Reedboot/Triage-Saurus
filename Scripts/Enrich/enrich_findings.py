...
def _load_cloud_posture(conn, experiment_id: str) -> dict | None:
    """Load the cloud posture JSON stored by _compute_cloud_posture() for this experiment.

    Returns the first posture found (most experiments have one repo), or None.
    """
    try:
        row = conn.execute(
            """
            SELECT cm.value FROM context_metadata cm
            WHERE cm.experiment_id = ? AND cm.namespace = 'cloud_posture' AND cm.key = 'posture_json'
...
def _cloud_posture_context_block(posture: dict) -> str:
    """Format cloud posture as a concise context block for LLM prompts."""
    if not posture:
        return ""

    lines = ["Cloud infrastructure context for this repository:"]

    if posture.get("behind_waf") and posture.get("behind_app_gateway"):
        lines.append(f"  - WAF-protected via App Gateway '{posture['app_gateway_name']}': YES — injection/XSS attack surface is reduced at the edge")
    elif posture.get("behind_app_gateway"):
        lines.append(f"  - Behind App Gateway '{posture['app_gateway_name']}' (no WAF): YES")
    else:
        lines.append("  - Direct internet exposure (no App Gateway / WAF detected): YES — edge attack surface is unreduced")

    if posture.get("behind_apim"):
        lines.append(f"  - APIM routing layer '{posture['apim_name']}': YES — API keys / OAuth layer present")
    else:
        lines.append("  - APIM routing: NO — API endpoints may be directly accessible")

    if posture.get("aks_cluster"):
        secured = posture.get("aks_secured")
        lines.append(
            f"  - AKS cluster '{posture['aks_cluster']}': YES, "
            + ("IP-restricted API server (authorizedIPRanges)" if secured else "API server is publicly accessible")
        )

    exposure = posture.get("endpoint_exposure", "unknown")
    if exposure == "public":
        lines.append("  - Endpoint exposure: FULLY PUBLIC (no IP restrictions detected)")
    elif exposure == "restricted":
        lines.append("  - Endpoint exposure: IP-RESTRICTED (partially exposed; not fully public)")
    elif exposure == "private":
        lines.append("  - Endpoint exposure: PRIVATE (no public endpoint)")
    else:
        lines.append("  - Endpoint exposure: unknown (no matching assets found)")

    auth = posture.get("auth_methods") or []
    if auth:
        lines.append(f"  - Auth methods on matched assets: {', '.join(auth)}")

    lines.append(
        "\nUse this context when scoring. If a finding is mitigated by the above controls "
        "(e.g. injection behind WAF, or secret exposure on a private endpoint), "
        "lower the severity_score accordingly. If controls are absent, raise it."
    )
    return "\n".join(lines) + "\n"


def _build_prompt(row: dict, cloud_posture: dict | None = None) -> str:
    ...
    posture_block = _cloud_posture_context_block(cloud_posture) if cloud_posture else ""
...
def main():
    ...
    if cloud_posture:
        print(f"[cloud-posture] Loaded infrastructure context: waf={cloud_posture.get('behind_waf')}, apim={cloud_posture.get('behind_apim')}, aks={cloud_posture.get('aks_cluster')}, exposure={cloud_posture.get('endpoint_exposure')}")

    print(f"Found {len(rows)} unenriched findings for experiment '{args.experiment}'")

    for row in rows:
        fid = row["id"]
        prompt = _build_prompt(row, cloud_posture=cloud_posture)
